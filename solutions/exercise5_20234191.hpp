...
private:
  ...
  std::vector<Eigen::MatrixXd> M_single_;
  vector<Eigen::VectorXd> b_single_;
...
public:
  ...
  void Initialize() {
    for (int i = 0; i < num_of_moving_joints_; i++){
      M_single_.push_back(Eigen::MatrixXd::Zero(6, 6));
      b_single_.push_back(Eigen::VectorXd::Zero(6));
  ...
  void CalArticulatedBodyInertia() {    
    ...
    for (int j = moving_joints_.size() - 1; j >= 0; j--){
      ...
      // calculate b_single_j
      b_single_[j].segment(0, 3) = child_link->mass_ * VecToSkew(joint_j->omega_cp_w_) * VecToSkew(joint_j->omega_cp_w_) * child_link->r_po_w_;
      b_single_[j].segment(3, 3) = VecToSkew(joint_j->omega_cp_w_) * (child_link->inertia_w_ - child_link->mass_ * VecToSkew(child_link->r_po_w_) * VecToSkew(child_link->r_po_w_)) * joint_j->omega_cp_w_;
      ...
      b_single_[j] = b_single_[j] - FT_by_gravity;

      // calculate M_single_j
      M_single_[j].block<3,3>(0, 0) = child_link->mass_ * Eigen::MatrixXd::Identity(3, 3);
      M_single_[j].block<3,3>(0, 3) = -child_link->mass_ * VecToSkew(child_link->r_po_w_);
      M_single_[j].block<3,3>(3, 0) = child_link->mass_ * VecToSkew(child_link->r_po_w_);
      M_single_[j].block<3,3>(3, 3) = child_link->inertia_w_ - child_link->mass_ * VecToSkew(child_link->r_po_w_) * VecToSkew(child_link->r_po_w_);

      // make twist
      Eigen::Matrix<double, 6, 1> twist_j;
      twist_j << joint_j->v_cp_w_ , joint_j->omega_cp_w_;
      ...
      if (j == leaf) {
        ...
        X.block<3,3>(3, 0) = VecToSkew(moving_joints_[j + 1]->r_pc_w_);
        X_dot.block<3,3>(3, 0) = VecToSkew(moving_joints_[j + 1]->v_cp_w_ - joint_j->v_cp_w_);
        S = moving_joints_[j + 1]->S_w_;
        S_dot.block<3,1>(3, 0) = moving_joints_[j + 1]->omega_c_w_.cross(moving_joints_[j + 1]->P_w_); // TODO: assuem only revolute joint

        M_a_[j] = M_single_[j] + X * M_a_[j + 1] * (-S / (S.transpose() * M_a_[j + 1] * S) * 
                  (S.transpose() * M_a_[j + 1] * X.transpose()) + X.transpose());
        b_a_[j] = b_single_[j] + X * (M_a_[j + 1] * (S / (S.transpose() * M_a_[j + 1] * S) * 
                  (gf_[j + 1] - S.transpose() * M_a_[j + 1] * (S_dot * gv_[j + 1] + X_dot.transpose() * twist_j) 
                  - S.transpose() * b_a_[j + 1]) + S_dot * gv_[j + 1] + X_dot.transpose() * twist_j) + b_a_[j + 1]);
    ...
    // calculate root
    // calculate b_single_j
    b_root_single_.segment(0, 3) = root->mass_ * VecToSkew(omega_b_w_) * VecToSkew(omega_b_w_) * root->r_po_w_;
    b_root_single_.segment(3, 3) = VecToSkew(omega_b_w_) * (root->inertia_w_ - root->mass_ * VecToSkew(root->r_po_w_) * VecToSkew(root->r_po_w_)) * omega_b_w_;

    Eigen::Matrix<double, 6, 1> FT_by_gravity;
    FT_by_gravity << root->mass_ * gravity_, VecToSkew(root->r_po_w_) * (root->mass_ * gravity_);

    b_root_single_ = b_root_single_ - FT_by_gravity;

    // calculate M_single_j
    M_root_single_.block<3,3>(0, 0) = root->mass_ * Eigen::MatrixXd::Identity(3, 3);
    M_root_single_.block<3,3>(0, 3) = -root->mass_ * VecToSkew(root->r_po_w_);
    M_root_single_.block<3,3>(3, 0) = root->mass_ * VecToSkew(root->r_po_w_);
    M_root_single_.block<3,3>(3, 3) = root->inertia_w_ - root->mass_ * VecToSkew(root->r_po_w_) * VecToSkew(root->r_po_w_);

    // make twist
    Eigen::Matrix<double, 6, 1> twist_j;
    twist_j << v_b_w_, omega_b_w_;

    M_root_a_ = M_root_single_;
    b_root_a_ = b_root_single_;
    vector<int> leg_moving_joint_idx[2] = {{0, 3, 6, 9}, {2, 5, 8, 11}};
    for (int j = 0; j < leg_moving_joint_idx[0].size(); j++){
      int leg_start_idx = leg_moving_joint_idx[0][j];
      Joint* joint_j = moving_joints_[leg_start_idx];

      Eigen::MatrixXd X = Eigen::MatrixXd::Identity(6, 6);
      Eigen::MatrixXd X_dot = Eigen::MatrixXd::Zero(6, 6);
      Eigen::VectorXd S = Eigen::VectorXd::Zero(6);
      Eigen::VectorXd S_dot = Eigen::VectorXd::Zero(6);
      X.block<3,3>(3, 0) = VecToSkew(joint_j->r_pc_w_);
      X_dot.block<3,3>(3, 0) = VecToSkew(joint_j->v_cp_w_ - v_b_w_);
      S = joint_j->S_w_;
      S_dot.block<3,1>(3, 0) = joint_j->omega_c_w_.cross(joint_j->P_w_); // TODO: assuem only revolute joint

      M_root_a_ = M_root_a_ + X * M_a_[leg_start_idx] * (-S / (S.transpose() * M_a_[leg_start_idx] * S)* 
                (S.transpose() * M_a_[leg_start_idx] * X.transpose()) + X.transpose());
      b_root_a_ = b_root_a_ + X * (M_a_[leg_start_idx] * (S / (S.transpose() * M_a_[leg_start_idx] * S) * 
                (gf_[leg_start_idx] - S.transpose() * M_a_[leg_start_idx] * (S_dot * gv_[leg_start_idx] + X_dot.transpose() * twist_j) 
                - S.transpose() * b_a_[leg_start_idx]) + S_dot * gv_[leg_start_idx] + X_dot.transpose() * twist_j) + b_a_[leg_start_idx]);
    }
  }
  ...
  void CalGeneralizedAcceleration() {
    ...
    a_b_w_ = (M_root_a_.inverse() * (gf_root_w_ - b_root_a_)).segment(0, 3);
    alpha_b_w_ = (M_root_a_.inverse() * (gf_root_w_ - b_root_a_)).segment(3, 3);
    ...
    for (int j = 0; j < num_of_moving_joints_; j++){
      ...
      X.block<3,3>(3, 0) = VecToSkew(joint_j->r_pc_w_);
      if (links_[joint_j->parent_link_idx_]->parent_joint_idx_ == -1){
        X_dot.block<3,3>(3, 0) = VecToSkew(joint_j->v_cp_w_ - v_b_w_);
        twist_p << v_b_w_, omega_b_w_;
        twist_dot_p << a_b_w_ , alpha_b_w_;
      }
      else{
        Joint* parent_joint = joints_[links_[joint_j->parent_link_idx_]->parent_joint_idx_];
        X_dot.block<3,3>(3, 0) = VecToSkew(joint_j->v_cp_w_ - parent_joint->v_cp_w_);
      ...
      ga_[j] = 1 / (S.transpose() * M_a_[j] * S) * (gf_[j] - S.transpose() * M_a_[j] * (S_dot * gv_[j] + X_dot.transpose() * twist_p + X.transpose() * twist_dot_p) 
                - S.transpose() * b_a_[j]);

      // calculate twist_dot_b
      twist_dot_b = X.transpose() * twist_dot_p + X_dot.transpose() * twist_p + S * ga_[j] + S_dot * gv_[j];
      joint_j->a_cp_w_ = twist_dot_b.segment(0, 3);
      joint_j->a_c_w_ = joint_j->a_cp_w_; // TODO: assuem only revolute joint
      joint_j->alpha_cp_w_ = twist_dot_b.segment(3, 3);
      joint_j->alpha_c_w_ = joint_j->alpha_cp_w_ - joint_j->omega_c_w_.cross(joint_j->P_w_) * gv_[j]; // TODO: assuem only revolute joint
    }
  }
  ...
  Eigen::MatrixXd GetGeneralizedAcceration(){
    ...
    ga.block<3, 1>(0, 0) = a_b_w_;
    ga.block<3, 1>(3, 0) = alpha_b_w_;
