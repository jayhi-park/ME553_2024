...
public:
  ArticulatedSystem(const Eigen::VectorXd& gc)
  {
    // set root
  ...
  void ForwardKinematics() {
    ...
    //   Link* child_link = links_[joint->child_link_idx_];
    //   cout << "joint " << joint->name_ << endl;
    //   cout << "parent link " << parent_link->name_ << endl;
    //   cout << "child link " << child_link->name_ << endl;
    //   cout << "R_wc_ " << joint->R_wp_ * joint->R_pc_ * joint->R_ccp_ << endl;
    //   cout << "P_w_ " << joint->R_wp_ << endl;
    //   cout << "r_pc_w_ " << joint->r_pc_w_ << endl;
    //   cout << "r_wc_w_ " << joint->r_wc_w_ << endl << endl;
    // }
    // exit(1);

    /*
    // cal root Jacobian
    root->Jacobian_p_ = Eigen::Matrix<double, 3, 18>::Zero();
    root->Jacobian_p_.block<3,3>(0, 0) = Eigen::Matrix3d::Identity();
    root->Jacobian_p_.block<3,3>(0, 3) = -VecToSkew(root->r_po_w_);

    for (int i = 0; i < moving_joints_.size(); i++){
      Joint* joint = moving_joints_[i];
      root->Jacobian_p_.block<3,1>(0, 6 + i) = joint->P_w_.cross(root->r_wo_w_ - joint->r_wc_w_);
    }

    root->Jacobian_a_ = Eigen::Matrix<double, 3, 18>::Zero();
    root->Jacobian_a_.block<3,3>(0, 3) = Eigen::Matrix3d::Identity();

    for (int i = 0; i < moving_joints_.size(); i++){
      Joint* joint = moving_joints_[i];
      root->Jacobian_a_.block<3,1>(0, 6 + i) = joint->P_w_;
    }

    // cal link Jacobian
    set<int> composite_links_idx;
    for (int i = 0; i < moving_joints_.size(); i++){
      composite_links_idx.insert(moving_joints_[i]->parent_link_idx_);
      composite_links_idx.insert(moving_joints_[i]->child_link_idx_);
    }


    for (auto i : composite_links_idx){
      Link* link = links_[i];

      link->Jacobian_p_ = Eigen::Matrix<double, 3, 18>::Zero();
      link->Jacobian_p_.block<3,3>(0, 0) = Eigen::Matrix3d::Identity();
      link->Jacobian_p_.block<3,3>(0, 3) = -VecToSkew(link->r_wo_w_ - r_wb_w_);

      for (int j = 0; j < moving_joints_.size(); j++){
        Joint* joint = moving_joints_[j];
        link->Jacobian_p_.block<3,1>(0, 6 + j) = joint->P_w_.cross(link->r_wo_w_ - joint->r_wc_w_);
      }

      link->Jacobian_a_ = Eigen::Matrix<double, 3, 18>::Zero();
      link->Jacobian_a_.block<3,3>(0, 3) = Eigen::Matrix3d::Identity();

      for (int j = 0; j < moving_joints_.size(); j++){
        Joint* joint = moving_joints_[j];
        link->Jacobian_a_.block<3,1>(0, 6 + j) = joint->P_w_;
      }
    }
    */
  }
  ...
  void CalMassMatrix() {
    ...
    for (int j = moving_joints_.size() - 1; j >= 0; j--){
      ...
      Eigen::VectorXd S_i_ = Eigen::VectorXd::Zero(6);
      ...
      while (parent_link->parent_joint_idx_ != -1){
        ...
        // cout << "i " << joint_i->moving_idx_ << endl;
        // cout << "parent_link " << parent_link->name_ << endl;
        // cout << "child_link " << child_link->name_ << endl;

        S_i_.block<3,1>(3, 0) = joint_i->P_w_;
        ...
        M_.block<1,1>(6 + j, 6 + joint_i->moving_idx_) = S_j_.transpose() * I * r * S_i_;
